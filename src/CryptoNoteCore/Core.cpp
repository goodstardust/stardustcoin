...
bool Core::getBlockTemplate(BlockTemplate& b, const AccountPublicAddress& adr, const BinaryArray& extraNonce,
  ...
  if (b.majorVersion == BLOCK_MAJOR_VERSION_1) {
    ...
    if (b.majorVersion >= BLOCK_MAJOR_VERSION_3) {
      ...
      // stardustc's own fork of check minorVersion to determine whether it is parent block
      // in this way we can bypass the issue
      b.parentBlock.minorVersion = 1;
    } else {
      // The code here is fucked up
...
std::error_code Core::validateBlock(const CachedBlock& cachedBlock, IBlockchainCache* cache, uint64_t& minerReward) {
  ...
  logger(Logging::DEBUGGING) << "Expected" << static_cast<int>(upgradeManager->getBlockMajorVersion(cachedBlock.getBlockIndex())) << " Submitted " << static_cast<int>(block.majorVersion);
